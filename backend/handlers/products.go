...
func GetProducts(db *sqlx.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		err := db.Select(&products, "SELECT * FROM product ORDER BY product_id ASC")
		if err != nil {
...
func CreateProduct(db *sqlx.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		query := `INSERT INTO Product (name, image, description, short_description, price, stock)
				  VALUES (:name, :image, :description, :short_description, :price, :stock) RETURNING product_id`

		rows, err := db.NamedQuery(query, &product)
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{
				"error":   "Ошибка добавления продукта",
				"details": err.Error(), // Логирование детали ошибки
			})
			return
		}
		if rows.Next() {
...
func UpdateProduct(db *sqlx.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		query := `UPDATE Product SET name = :name, image = :image, description = :description, 
		short_description = :short_description, price = :price, stock = :stock 
