...
import 'package:flutter_market_alpha/pages/pick_receiver.dart';
...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _fetchUser() async {
    try {
      final user =
          await ApiService().getUser(authService.getCurrentUserEmail() ?? "");
      setState(() {
        _user = user;
        _isLoading = false;
      });
                    ...
                    GestureDetector(
                      onTap: () {
                        Navigator.of(context).push(
                          MaterialPageRoute(
                            builder: (context) =>
                                MyOrders(userId: globalProfileId),
                          ),
                        );
                      },
                      child: Container(
                    ...
                    ElevatedButton(
                      onPressed: () {
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                              builder: (context) => const PickReceiver()),
                        );
                      },
                      style: ElevatedButton.styleFrom(
                          shape: RoundedRectangleBorder(
                      ...
                      child: const Text('Просмотреть чаты'),
                    ),
                  ],
                ),
    );
